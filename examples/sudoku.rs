...
fn to_board(res: SelectedDependencies<DP>) -> String {
    ...
    for row in 1..=BOARD_SIZE {
        for col in 1..=BOARD_SIZE {
            out.push_str(&res[&SudokuPackage::Cell { row, col }].to_string());
            out.push(' ');
            if col % BOARD_BASE == 0 && col != BOARD_SIZE {
                out.push_str("| ");
            }
        }
        out.push('\n');
        if row % BOARD_BASE == 0 && row != BOARD_SIZE {
            out.push_str("------+-------+-------\n");
        }
    }
    out
...
fn encode_constraints(
    dependency_provider: &mut OfflineDependencyProvider<SudokuPackage, Range<u8>>,
) {
    for row in 1..=BOARD_SIZE {
        for col in 1..=BOARD_SIZE {
...
fn main() {
    ...
    match resolve(&dependency_provider, SudokuPackage::Root, 0) {
    ...
    println!("Time taken: {:0.5}s", start.elapsed().as_secs_f32());
    ...
    println!("Time taken: {:.5}s", start.elapsed().as_secs_f32());
}
