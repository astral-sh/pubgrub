...
pub trait VersionSet: Debug + Display + Clone + Eq + Hash {
    ...
    ///
    /// The default is equivalent to `self.intersection(&other.complement())`. Version sets that
    /// carry candidate-selection metadata can override this operation to keep metadata from the
    /// left-hand requirement without inheriting metadata from the excluded right-hand set.
