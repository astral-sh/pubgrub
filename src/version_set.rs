...
pub trait VersionSet: Debug + Display + Clone + Eq + Hash {
    ...
    ///
    /// PubGrub uses this method when deciding whether dependency constraints can be merged and
    /// whether a logically redundant constraint changes the candidates chosen by the dependency
    /// provider. The default assumes candidate selection depends only on version membership.
    /// Implementations that carry additional selection metadata must override it.
    ///
